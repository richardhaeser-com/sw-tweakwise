...
### Fixed
-  Make sure a csrf token is available on Shopware 6.4 instances so it is possible to use the add-to-cart function on Shopware 6.4.x as well.
...
### Added
- Added the stock of other variants to the feed as well
...
- Does not load products in background on product listing while the Tweakwise JS implementation is used.
